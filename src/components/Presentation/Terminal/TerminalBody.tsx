...
const Command = styled.p<any>((props) => {
  return css`
    position: relative;
    width: max-content;
    font-weight: 400;
    ...
    &:before {
      ...
      animation: ${typewriter(props._theme)} 1s steps(${props.steps}) ${props.nCommand}s forwards;
    }
    &:after {
      width: 1ch;
      animation: ${typewriter2(props._theme)} 1s steps(${props.steps}) ${props.nCommand}s forwards,
        ${blink(props._theme)} 750ms ${props.nCommand + 1}s ${props.last ? 'infinite' : '0'}
          forwards;
    }
  `;
});

const TerminalBodyComponent = styled.div<any>((props) => {
  return css`
    padding: 0.4rem;
    display: flex;
